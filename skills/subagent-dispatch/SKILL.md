...
process is assumed to be `Opus`; code-writing subagents are
`Sonnet`; read-only discovery/lookup subagents (file/symbol
triage before a brief is written) are `Haiku`.
...
- [ ] **Acceptance defined**: know the shell command(s) to run against the diff before spending Opus review tokens.
- [ ] **No local build by default — CI owns compile/verify**: if CI runs `npm run build` / `npm test` on every PR, briefs say "do not build locally." Orchestrator builds locally ONLY to reproduce a CI failure (see "Build verification").
...
Before writing briefs for a fan-out, run a `Haiku` discovery pass (read-only: find the files/symbols, don't edit) so every brief pins real paths + line numbers instead of telling a subagent to "find the file."
...
BRANCH:
- Fresh from main: `git checkout -b <type>/<task-id>-<unique-suffix> main`
- Exists (crashed prior run) -> delete first: `git branch -D <branch> 2>/dev/null`

BUILD/CI GATE (CRITICAL):
- You do NOT build. Finish edits, run `npm test` and `npm run lint`, commit
  incrementally, push, open PR. CI runs `npm run build`. CI red -> orchestrator
...
ACCEPTANCE (orchestrator verifies before Opus review):
...
- **Default: subagents don't build, orchestrator doesn't pre-build.** Subagents edit → test (`npm test`) → commit → push → PR. CI builds (`npm run build`).
...
- **Tests are the exception if CI doesn't run them** — keep logic coverage local (pre-push smoke + test-before-PR). Otherwise let CI run `npm test` too.
...
```bash
git worktree add <verifier-path> origin/main --detach
...
npm run build
npm test   # scope to the failing suite if the tool supports it
```
...
2. **Independent review** (`subagent_type: "code-reviewer"`, `model: "Opus"`). Must differ from `Sonnet` — independence matters.
...
5. **After merge, sanity-check `main`'s copy of the file** before starting the next PR that touches it; broken -> fix the base branch first.
