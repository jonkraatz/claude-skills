...
- **Diff-scoped.** Everything reviewed and touched must be inside the diff
  against `{{BASE_BRANCH}}`. Don't wander into untouched files.
- **Must stay green.** Every change made here is re-proven by the same gates
  Phase A of `/fix` uses — no exceptions.

## 1. Scope the diff

Compute the changed surface: `git diff {{BASE_BRANCH}}...HEAD` (or the PR's
...
Dispatch a **{{MODEL_CODE_WRITER}}** subagent per logical cleanup (or do it
...
confirmation that `{{BUILD_CMD}}` and `{{TEST_CMD}}` are green. If a real bug
