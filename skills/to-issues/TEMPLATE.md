...
The issue tracker (**{{TRACKER_KIND}}**) and triage label vocabulary should have
been provided to you — run **`/install-engineering-loops`** if not.
...
an issue reference (issue number, URL, or path) as an argument, fetch it from
the issue tracker and read its full body and comments.
...
<!-- SLOT: domain-docs -->
No project domain docs configured. If this repo has a domain glossary or
architecture decision records, use their vocabulary in issue titles/
descriptions and respect the decisions in the area you're touching.
<!-- /SLOT -->
...
(**{{TRACKER_KIND}}**). Use the issue body template below.

<!-- SLOT: filing-template -->
Draft each issue from the template, show it, and file on confirmation via the
...
<!-- SLOT: tag-taxonomy -->
No canonical→repo label map configured. If this tracker has a "ready for
agent" / "ready for AFK" label or equivalent status, apply it; otherwise
publish without a triage label and note that in your summary.
<!-- /SLOT -->
