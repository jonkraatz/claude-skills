...
any edit: `git checkout -b {{BRANCH_PREFIX_FEATURE}}<slug>-plan` from
`{{BASE_BRANCH}}` (or `EnterWorktree` in a background job). No implementation
...
(model: {{MODEL_LOOKUP}}) over the codebase to confirm where the feature would
...
subagent on **{{MODEL_REVIEWER}}** (which must differ from the code-writer
model, {{MODEL_CODE_WRITER}}, so the review is a genuine second perspective).
...
After sign-off:
1. **`/to-prd`** — publish the PRD to the issue tracker ({{TRACKER_KIND}}).
...
**{{MODEL_LOOKUP}}**; the adversarial reviewer runs on **{{MODEL_REVIEWER}}**.
...
- **`code-reviewer`** on **{{MODEL_REVIEWER}}** — the mandatory independent
